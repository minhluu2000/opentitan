...
`elsif BUGNUMLULADAPRSRAM3
  if (EnableReadback) begin : gen_cmd_readback_check
    ...
    logic unused_sram_byte_readback_error;
    assign unused_sram_byte_readback_error = '1;
    assign readback_error = '0;
    assign readback_error_q = '0;
  end
`elsif BUGNUMLULADAPRSRAM1T
  if (EnableReadback) begin : gen_cmd_readback_check
    assign readback_error = sram_byte_readback_error;
    // permanently latch readback error until reset
    always_ff @(posedge clk_i or negedge rst_ni) begin
...
`endif
  ...
  assign instr_error = prim_mubi_pkg::mubi4_test_invalid(
      tl_i.a_user.instr_type
  ) | (prim_mubi_pkg::mubi4_test_true_strict(
      tl_i.a_user.instr_type
...
`ifdef BUGNUMLULADAPRSRAM8
  // assign a_ack    = tl_i_int.a_valid & tl_o_int.a_ready;
  assign d_ack    = tl_o_int.d_valid & tl_i_int.d_ready;
  assign sram_ack = req_o & gnt_i;
`elsif BUGNUMLULADAPRSRAM9
  assign a_ack    = tl_i_int.a_valid & tl_o_int.a_ready;
  assign d_ack    = tl_o_int.d_valid & tl_i_int.d_ready;
  assign sram_ack = '0;
`elsif BUGNUMLULADAPRSRAM4T
  assign d_ack    = tl_i_int.a_valid & tl_o_int.a_ready;
  assign a_ack    = tl_o_int.d_valid & tl_i_int.d_ready;
  assign sram_ack = req_o & gnt_i;
`else
  assign a_ack    = tl_i_int.a_valid & tl_o_int.a_ready;
  assign d_ack    = tl_o_int.d_valid & tl_i_int.d_ready;
...
`endif
  logic vld_rd_rsp;
`ifdef BUGNUMLULADAPRSRAM12
  assign vld_rd_rsp = '0;

  logic [top_pkg::TL_DW-1:0] error_blanking_data;
  assign error_blanking_data = (prim_mubi_pkg::mubi4_test_true_strict(
      reqfifo_rdata.instr_type
  )) ? DataWhenInstrError : DataWhenError;

`elsif BUGNUMLULADAPRSRAM7T
...
`ifdef BUGNUMLULADAPRSRAM13
  logic [DataIntgWidth-1:0] error_blanking_integ;
  assign error_blanking_integ = (prim_mubi_pkg::mubi4_test_true_strict(
      reqfifo_rdata.instr_type
  )) ? error_instr_integ : error_data_integ;

  logic [top_pkg::TL_DW-1:0] d_data;
  assign d_data = (vld_rd_rsp & ~d_error) ? rspfifo_rdata.data : error_blanking_data;
  logic [DataIntgWidth-1:0] data_intg;
  assign data_intg = (vld_rd_rsp && reqfifo_rdata.error) ? error_blanking_integ :
      (vld_rd_rsp) ? 0 :
      prim_secded_pkg::SecdedInv3932ZeroEcc;


`elsif BUGNUMLULADAPRSRAM8T
  logic [DataIntgWidth-1:0] error_blanking_integ;
  assign error_blanking_integ = (prim_mubi_pkg::mubi4_test_true_strict(
      reqfifo_rdata.instr_type
  )) ? error_instr_integ : error_data_integ;
...
`ifdef BUGNUMLULADAPRSRAM15
  assign req_o = tl_i_int.a_valid & '0 & ~error_internal;
...
`elsif BUGNUMLULADAPRSRAM16
  assign req_o  = tl_i_int.a_valid & reqfifo_wready & ~error_internal;
  // assign req_type_o = tl_i_int.a_user.instr_type;
  assign we_o   = tl_i_int.a_valid & (tl_i_int.a_opcode inside {PutFullData, PutPartialData});
  assign addr_o = (tl_i_int.a_valid) ? tl_i_int.a_address[DataBitWidth+:SramAw] : '0;
`elsif BUGNUMLULADAPRSRAM17
  assign req_o = tl_i_int.a_valid & reqfifo_wready & ~error_internal;
  assign req_type_o = tl_i_int.a_user.instr_type;
  assign we_o = tl_i_int.a_valid & ~(tl_i_int.a_opcode inside {PutFullData, PutPartialData});
  assign addr_o = (tl_i_int.a_valid) ? tl_i_int.a_address[DataBitWidth+:SramAw] : '0;
`elsif BUGNUMLULADAPRSRAM10T
  assign req_o = tl_i_int.a_valid & reqfifo_wready & ~error_internal;
  assign req_type_o = tl_i_int.a_user.instr_type;
  assign we_o = tl_i_int.a_valid & (tl_i_int.a_opcode inside {PutFullData, PutPartialData});
  assign addr_o = (tl_i_int.a_valid) ? '1 : '0;
`elsif BUGNUMLULADAPRSRAM11T
  assign req_o = tl_i_int.a_valid & reqfifo_wready & ~error_internal;
  assign req_type_o = tl_i_int.a_user.instr_type;
  //assign we_o = tl_i_int.a_valid & (tl_i_int.a_opcode inside {PutFullData, PutPartialData});
  assign addr_o = (tl_i_int.a_valid) ? tl_i_int.a_address[DataBitWidth+:SramAw] : '0;
...
`endif
  ...
  always_comb begin
    wmask_intg = '0;
    wdata_intg = '0;
`ifdef BUGNUMLULADAPRSRAM18
    if (tl_i_int.a_valid) begin
      wmask_intg[woffset] = {DataIntgWidth{1'b0}};
      wdata_intg[woffset] = tl_i_int.a_user.data_intg;
    end
`elsif BUGNUMLULADAPRSRAM19
    if (tl_i_int.a_valid) begin
      // wmask_intg[woffset] = {DataIntgWidth{1'b1}};
      wdata_intg[woffset] = tl_i_int.a_user.data_intg;
    end
`elsif BUGNUMLULADAPRSRAM12T
    if (tl_i_int.a_valid) begin
      wmask_intg[woffset] = {DataIntgWidth{1'b1}};
      wdata_intg[woffset] = '0;
    end
`elsif BUGNUMLULADAPRSRAM13T
    if (tl_i_int.a_valid) begin
      wmask_intg[woffset] = {DataIntgWidth{1'b1}};
      // wdata_intg[woffset] = tl_i_int.a_user.data_intg;
    end
...
`elsif BUGNUMLULADAPRSRAM21
  for (genvar i = 0; i < WidthMult; i++) begin : gen_write_output
    if (EnableDataIntgPt) begin : gen_combined_output
      assign wdata_combined[i] = {wmask_intg[i], wmask_int[i]};
      assign wmask_combined[i] = {wdata_intg[i], wdata_int[i]};
    end else begin : gen_ft_output
      logic unused_w;
      assign wmask_combined[i] = wmask_int[i];
      assign wdata_combined[i] = wdata_int[i];
...
`ifdef BUGNUMLULADAPRSRAM27
  //assign reqfifo_wvalid = a_ack;

`elsif BUGNUMLULADAPRSRAM14T
  assign reqfifo_wvalid = '0;
...
`endif
  assign reqfifo_wdata = '{
          op:
          (
          tl_i_int.a_opcode != Get
          ) ?
          OpWrite
          :
          OpRead,  // To return AccessAck for opcode error
...
`ifdef BUGNUMLULADAPRSRAM30
  //assign sramreqfifo_wdata = '{mask    : tl_i_int.a_mask, woffset : woffset};
  assign sramreqfifo_wvalid = sram_ack & ~we_o;
  assign sramreqfifo_rready = rspfifo_wvalid;

  assign rspfifo_wvalid = rvalid_i & reqfifo_rvalid;
`elsif BUGNUMLULADAPRSRAM31
  assign sramreqfifo_wdata = '{mask    : tl_i_int.a_mask, woffset : woffset};
  assign sramreqfifo_wvalid = '0 & ~we_o;
  assign sramreqfifo_rready = rspfifo_wvalid;
...
`ifdef BUGNUMLULADAPRSRAM32
  assign rspfifo_rready = (reqfifo_rdata.op == OpRead & ~reqfifo_rdata.error) ? '0 : 1'b0;
`elsif BUGNUMLULADAPRSRAM33
  assign rspfifo_rready = (reqfifo_rdata.op != OpRead & ~reqfifo_rdata.error)
                        ? reqfifo_rready : 1'b0 ;
`elsif BUGNUMLULADAPRSRAM15T
  // assign rspfifo_rready = (reqfifo_rdata.op == OpRead & ~reqfifo_rdata.error)
  //                     ? reqfifo_rready : 1'b0 ;
`else
  assign rspfifo_rready = (reqfifo_rdata.op == OpRead & ~reqfifo_rdata.error)
